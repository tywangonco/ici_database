...
ici_database = {
    "非小細胞肺癌 (NSCLC)": {
        "鞏固治療": {
            ...
            "臨床條件": "第三期局部晚期、無法手術切除。非鱗狀需 EGFR/ALK/ROS-1 原生型，鱗狀需 EGFR/ALK 原生型。需於根治性同步放射治療合併至少2個週期[...]"
        },
        "第一線用藥 (單用)": {
        ...
        "第三線用藥 (肺腺癌)": {
            ...
            "單用/併用": "單獨使用 [條文：9.69-1]",
            "PD_L1_條件": "pembrolizumab: TPS >= 50%; nivolumab: TC >= 50%; atezolizumab: TC >= 50% 或 IC >= 10% [條文：9.69-3-(3)附表]",
            "臨床條件": "先前已使用過 platinum 類及 docetaxel/paclitaxel 類二線(含)以上化療均失敗，又有疾病惡化，且 EGFR/ALK/ROS-1 原生型之晚期非小細胞肺[...]"
        }
    },
    "晚期肝細胞癌 (HCC)": {
        "第一線用藥": {
            ...
            "PD_L1_條件": "不需檢附報告 [條文：9.69-3-(3)附表]",
            "臨床條件": "Child-Pugh A class [條文：9.69-1-(8)]。未曾接受全身性療法 [條文：9.69-2-(1)]。需具備肝外轉移、大血管侵犯，或經 T.A.C.E. 於12個月內 >= [...]"
        },
        "後續治療 (舊案續用)": {
            "藥物": ["nivolumab"],
            "單用/併用": "單獨使用 [條文：9.69-1]",
    ...
    "胃癌": {
        "第一線用藥": {
            ...
            "臨床條件": "第一線治療晚期或轉移性且不具有 HER2 過度表現的胃癌病人（不含胃腸基質瘤及神經內分泌腫瘤/癌）。與 zolbetuximab 僅得擇一[...]"
        },
        "後線治療 (舊案續用)": {
            "藥物": ["nivolumab"],
            "單用/併用": "單獨使用 [條文：9.69-1]",
    ...
    "膽道癌": {
        "第一線用藥": {
            ...
            "臨床條件": "先前未接受過治療或不可手術之局部晚期或轉移性膽道癌 [條文：9.69-2-(6)]。須排除壺腹癌、曾接受異體器官移植、具有或曾有[...]"
        }
    },
    "泌尿道上皮癌": {
        "第一線用藥 (單用)": {
            ...
            "臨床條件": "轉移性且不適合接受化學治療 [條文：9.69-1-(4)]。需符合聽力受損、周邊神經病變或 CIRS score > 6 條件之一 [條文：9.69-1-(4)]。pemb[...]"
        },
        "第一線用藥 (併用)": {
            "藥物": ["nivolumab"],
        ...
        "第二線用藥": {
            ...
            "臨床條件": "先前已使用過 platinum 類化學治療失敗後疾病惡化的局部晚期無法切除或轉移性患者 [條文：9.69-1-(4)]。eGFR > 30 mL/min/1.73m2 [條文[...]"
        },
        "維持療法": {
            "藥物": ["avelumab"],
            "單用/併用": "單獨使用 [條文：9.69-1]",
            "PD_L1_條件": "不需檢附報告 [條文：9.69-3-(3)附表]",
            "臨床條件": "接受第一線含鉑化學治療4至6個療程後，疾病未惡化且達部分緩解 (PR) 或呈穩定狀態 (SD) [條文：9.69-1-(4)]。eGFR > 30 mL/min/1.73m2 [[[...]"
        }
    },
    "頭頸部鱗狀細胞癌 (不含鼻咽癌)": {
        "第一線用藥": {
    ...
    "黑色素瘤": {
        "晚期/轉移性": {
            ...
            "單用/併用": "單獨使用 [條文：9.69-1]",
            "PD_L1_條件": "不需檢附報告 [條文：9.69-3-(3)附表]",
            "臨床條件": "腫瘤無法切除或轉移之第三期或第四期，先前曾接受過至少一次全身性治療失敗者 [條文：9.69-1-(1)]。需檢附 BRAF 腫瘤基因檢測[...]"
        }
    },
    "早期三陰性乳癌": {
        "術前前導至術後輔助": {
            "藥物": ["pembrolizumab"],
            "單用/併用": "併用接單用 (前導：與 carbo/paclitaxel 併用至多4療程 -> 加上 cyclo/doxo 或 epirubicin 併用至多4療程；輔助：單用至多9療程) [條文：[...]",
            "PD_L1_條件": "不需檢附報告 [條文：9.69-3-(3)附表]",
            # 臨床條件加入 pembrolizumab 與 olaparib 僅能擇一支付的限制[cite: 2]
            "臨床條件": "非轉移性、第II期至第IIIb期成年病人 [條文：9.69-2-(7)]。初次申請需檢附 ER、PR 及 HER2 為陰性之檢測報告 [條文：9.69-3-(7)]。術[[...]"
        }
    },
    "小細胞肺癌": {
        "擴散期": {
...
def format_citation(text):
    """使用正規表達式將 [第 X 項] 和 [條文：XXX] 轉換為美化的 Markdown 標籤"""
    if not text:
        return ""
    # Fixed: Corrected regex pattern to properly match [第 \d+ 項]
    text = re.sub(r'\[第\s*(\d+)\s*項\]', r'**[健保給付規定第 \1 項]**', text)
...
def main():
    ...
    st.warning(
        "**⚠️ 全域給付重要提醒 & 排除條件**\n"
        "1. **給付時程期限**：原則上自初次處方用藥日起算 **2 年**（例外：durvalumab 用於非小細胞肺癌鞏固治療為 1 年；pembrolizumab 用於早期三陰性乳[...]"
        "2. **互換與合併限制**：各適應症限給付一種 ICI 藥物，且**不得互換**。治療期間**不可合併申報標靶藥物**（例外：atezolizumab+bevacizumab 用於 HC[...]"
    ...
    st.markdown("---")
    st.caption("📝 免責聲明：本系統依據使用者提供之資料庫整理，實際給付規定及詳細限制請以中央健康保險署最新公告之「全民健康保險藥物給付[...]")
